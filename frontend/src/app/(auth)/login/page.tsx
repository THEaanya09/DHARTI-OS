...
export default function LoginPage() {
  ...
  const loginText = dictionary.auth.login;
  const { login, loginWithGoogle, loginWithGithub, loading, loadingMessage } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Errors are handled inside useAuth toast
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-background bg-noise">
      {/* Left: Brand panel */}
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-primary/10 via-background to-info/5 border-r border-border/50">
        <div className="absolute inset-0 bg-grid opacity-30" />
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 h-[400px] w-[400px] rounded-full bg-primary/10 blur-[120px] pointer-events-none float-glow" />
        <div className="relative flex flex-col justify-center px-16 z-10 space-y-12">
          ...
          <div>
            <h2 className="text-display max-w-lg font-display text-foreground leading-tight tracking-tight">
              Climate decisions should be{' '}
              <span className="gradient-text">intelligent</span>, not intuitive.
            </h2>
            <p className="mt-6 max-w-md text-lg text-muted-foreground leading-relaxed">
              Join thousands of farmers and government officials making data-driven climate decisions.
            </p>
          </div>
      ...
      <div className="flex w-full items-center justify-center px-6 lg:w-1/2">
        ...
        >
          ...
          <motion.div variants={fadeInUp}>
            <h1 className="text-heading-1">{loginText.title}</h1>
            <p className="mt-2 text-body text-muted-foreground">{loginText.description}</p>
          </motion.div>

          <motion.form variants={fadeInUp} className="mt-8 space-y-5" onSubmit={handleSubmit}>
            <div className="space-y-2">
              <Label htmlFor="email">{loginText.email}</Label>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Link href="#" className="text-caption text-primary hover:text-primary/80 transition-colors">
                  {loginText.forgot}
                </Link>
              </div>
              <div className="relative">
