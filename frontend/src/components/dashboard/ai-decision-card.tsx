...
const priorityConfig: Record<InsightPriority, { icon: ReactNode; color: string }> = {
  critical: { icon: <AlertTriangle className="h-4 w-4" />, color: 'text-destructive' },
  high: { icon: <AlertCircle className="h-4 w-4" />, color: 'text-warning' },
  medium: { icon: <Info className="h-4 w-4" />, color: 'text-muted-foreground' },
  low: { icon: <CheckCircle className="h-4 w-4" />, color: 'text-muted-foreground' },
};

export function AIDecisionCard() {
  const { dictionary } = useI18n();
  const ai = dictionary.dashboard.ai;
  const { insights, loading, hasLiveData } = useIntelligence();

  if (loading && !hasLiveData) {
    return <CardLoading message="Generating AI recommendations…" />;
  }

  if (insights.length === 0) {
    return <CardEmpty message="No recommendations yet. Run analysis to get live AI insights." />;
  }
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          {insights.map((insight) => {
            ...
            return (
              <div key={insight.id} className="border-b border-border/20 last:border-0 pb-4 last:pb-0">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0 space-y-1.5">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-[10px] font-mono font-bold text-primary">{insight.confidence}% Match</span>
                    </div>
